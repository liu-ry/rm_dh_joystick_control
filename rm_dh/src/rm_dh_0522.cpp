...
// this version is send by two thread
using namespace std;
...
Dh_Gripper::Dh_Gripper(std::string name) : Node(name)
{
    ...
    else
    {
        ...
        if(0 == model_flag)
        {
            ...
            //启动action_joint_state线程
            action_joint_state_thread_ = std::thread(&Dh_Gripper::publish_action_joint_states_thread, this);
        }
...
void Dh_Gripper::actionJointCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
    ...
    res = Rm_Api_.Service_Write_Single_Register(m_sockhand_, rm_port_, dh_position_addr_, msg->position[6], dh_addr_, block_);
    if (0 == res)
    {
        RCLCPP_INFO(this->get_logger(), "******** Successfully ********");
    }
    else
    {
        ...
        std::cout << msg->position[6] << std::endl;
    }
}

void Dh_Gripper::publish_action_joint_states_thread()
{
    ...
    while (rclcpp::ok())
    {
        ...
        // RCLCPP_INFO(this->get_logger(), "Current time (nanoseconds): %ld", now.nanoseconds());
        {
            std::lock_guard<std::mutex> lock(action_data_mutex);
            // 读取机械臂joint的数据
            joint_action_state_msg_.position[0] = joint_read_[0];
            joint_action_state_msg_.position[1] = joint_read_[1];
            joint_action_state_msg_.position[2] = joint_read_[2];
            joint_action_state_msg_.position[3] = joint_read_[3];
            joint_action_state_msg_.position[4] = joint_read_[4];
            joint_action_state_msg_.position[5] = joint_read_[5];
...
void Dh_Gripper::publish_joint_states_thread()
{
    ...
    while (rclcpp::ok())
    {
        ...
        if (0 == res)
        {
            ...
            // 读取机械臂的位置（以米为单位)
            pose_msg_.position.x = current_pose.position.x;
            pose_msg_.position.y = current_pose.position.y;
            pose_msg_.position.z = current_pose.position.z;

            // 设置机械臂的姿态（以欧拉角表示）
            pose_msg_.orientation.x = current_pose.euler.rx;
            pose_msg_.orientation.y = current_pose.euler.ry;
            pose_msg_.orientation.z = current_pose.euler.rz;
